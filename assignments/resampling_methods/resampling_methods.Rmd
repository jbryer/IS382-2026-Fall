...
2. We will now consider the `Boston` housing data set, from the `ISLR2` package.
...
(f) We now would like to estimate the standard error of $\hat{\mu}_{med}$µmed. Unfortunately, there is no simple formula for computing the standard error of the median. Instead, estimate the standard error of the median using the bootstrap. Comment on your findings.
